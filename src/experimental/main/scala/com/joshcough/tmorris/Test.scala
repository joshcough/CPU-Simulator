  ...
  object Exercises {
    ...
    def add(x: Natural, y: Natural): Natural = {
      (x,y) match {
        case (Zero,y) => y
	case (x,Zero) => x
	case (Succ(_), Succ(j)) => add( Succ(x), j )
      }
    }
    ...
    // Elegance: 0.5 marks
    // Total: 4
    def sum(is: List[Int]): Int = List.foldLeft(is, 0, (sum:Int, i:Int) => sum + i )
    ...
    // Elegance: 0.5 marks
    // Total: 4
    def length[A](as: List[A]): Int = List.foldLeft(as, 0, (sum:Int, a:A) => sum + 1 )
    ...
    // Elegance: 1.5 marks
    // Total: 7
    def map[A, B](as: List[A], f: A => B): List[B] = List.foldLeft(as, Empty, (bs:List[B], a:A) => Cons(f(a), bs))
