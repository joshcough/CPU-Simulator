...
import com.joshcough.cpu.{AllSwitchBitList, BitList}
import com.joshcough.cpu.electric.{BasePowerSource, PowerSource, State}
import State._
...
trait RAM extends Addressable {
  ...
  val dataOut: BitList
  val numberOfRegisters = scala.math.pow(2, addressInputs.size).toInt
...
case class RealRAM(val dataIn: BitList,
  ...
  var selector = Selector(addressInputs, registers.map(_.output.powerSources))
...
//  val registers = new scala.collection.mutable.ListBuffer[String]
...
//  class Updater extends BasePowerSource(Nil) {
//
//    def calculateNewState(inputs: List[PowerSource]) = out.calculateNewState(inputs)
//
